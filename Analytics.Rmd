...
### By: Yiwen Yang, Zhiyi Wang, Ying Yang, Qiqi Wang
...
Then, the following charts demonstrate the race and sex distribution in our sample. The majority of our sample is White. The number of boys and girls is basically the same.
...
Moreover, we want to learn the household income level of our sample. The average is about 250, which means the average household income level is about 2.5 times of poverty threshold. The boxplot shows that 25% of households are under 1.25 times of poverty threshold, 50% of households are under 2.36 times of poverty threshold, 75% of households are under 3.89 times of poverty threshold.
...
Firstly, we looked at the effect of the sexual distinction on school attendance. There are 48.8% of the female attending the school, compared to 48% of the male. So, it is almost the same chance for parents to send their children to school.
...
Then, according to the analysis between the school attendance rate and the students’ races, Black are most likely to send their children to school (57.3%), followed by the native and the Asian or Island (both for 49.1%). Then the chances for the white, the multiracial population and the others are 48.1%, 47.4% and 36.9% respectively. 
...
The following bar chart shows the school attendance rates in each education level. Overall, the higher parents’ highest education level is, the more likely are children to go to school.
...
We used the logit model to analyze effects of race, sex, income, and education on children’s school attendance. The result shows that parents’ highest education and household income level have a significant correlation with the dependent variable.
...
- Sex is not significant at the 5% level.
...
[Go back to REPO to check R code](https://github.com/ps0205vi/Evans599FinalProject)
